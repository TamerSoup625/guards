...
You can check the documentation for explanations of all the module's features.
...
- Support for `if let`-like syntax and pattern matching
...
- Methods for basic function chaining
...
- `outcome_do` notation for more complex optional chaining
...
- Extensive type annotations and type guards support
...
- `force_guard` decorator for functions which are more likely to raise an exception and `MustUse` object to ensure these exceptions are handled
